...
import jetbrains.mps.build.mps.behavior.BuildMps_AbstractModule_BehaviorDescriptor;
...
public class check_BuildSolutionRunnerAspect_NonTypesystemRule extends AbstractNonTypesystemRule_Runtime implements NonTypesystemRule_Runtime {
  ...
  public void applyRule(final SNode buildSolutionRunner, final TypeCheckingContext typeCheckingContext, IsApplicableStatus status) {
    final String className = BuildSolutionRunnerAspect_BehaviorDescriptor.getClassName_id1aYLt$9dW4N.invoke(buildSolutionRunner);
    final String methodName = BuildSolutionRunnerAspect_BehaviorDescriptor.getMethodName_id1aYLt$9dX0a.invoke(buildSolutionRunner);
    SModuleReference moduleReference = ModuleReference.parseReference(BuildMps_AbstractModule_BehaviorDescriptor.getModuleReference_id41K1b4v5ZCB.invoke(SLinkOperations.getTarget(buildSolutionRunner, MetaAdapterFactory.getReferenceLink(0x427a473d5177432cL, 0x9905bcbceb71b996L, 0x39ea87a41cc0827eL, 0x54b085b5945c6691L, "solution"))));
    ...
    if (module != null) {
      for (SModel m : module.getModels()) {
        if (SModelOperations.getModelName(m).equals(module.getModuleName())) {
          ...
          SNode methodToRun = Sequence.fromIterable(ClassConcept_BehaviorDescriptor.staticMethods_id4_LVZ3pCeXr.invoke(classToRun)).findFirst(new IWhereFilter<SNode>() {
            public boolean accept(SNode it) {
