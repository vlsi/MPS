...
if [ -n "$MPS_JDK" -a -x "$MPS_JDK/bin/java" ]; then
  JDK="$MPS_JDK"
elif [ -s "$HOME/.MPS2017.1/config/idea.jdk" ]; then
  JDK=`"$CAT" $HOME/.MPS2017.1/config/idea.jdk`
  if [ ! -d "$JDK" ]; then
    JDK="$IDE_HOME/$JDK"
  fi
...
elif [ -r "$HOME/.MPS2017.1/mps$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.MPS2017.1/mps$BITS.vmoptions"
...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$IDE_BIN_HOME/lib$AGENT_LIB.so" ]; then
    AGENT="-agentlib:$AGENT_LIB=disablealloc,delay=10000,sessionname=MPS2017.1"
  fi
fi
