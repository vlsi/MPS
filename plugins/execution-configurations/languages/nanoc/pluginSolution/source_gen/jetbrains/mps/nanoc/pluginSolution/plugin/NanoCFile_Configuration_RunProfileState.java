...
public class NanoCFile_Configuration_RunProfileState extends DebuggerRunProfileState implements RunProfileState {
  @NotNull
  private final NanoCFile_Configuration myRunConfiguration;
  @NotNull
  private final ExecutionEnvironment myEnvironment;

  public NanoCFile_Configuration_RunProfileState(@NotNull NanoCFile_Configuration configuration, @NotNull Executor executor, @NotNull ExecutionEnvironment environment) {
  ...
  public ExecutionResult execute(Executor executor, @NotNull ProgramRunner runner) throws ExecutionException {
    ...
    File file = new File(Gcc_Command.getExecutableFile(SNodeOperations.cast(myRunConfiguration.getNode().getNode(), "jetbrains.mps.nanoc.structure.File")).getAbsolutePath());
    if (myDebuggerSettings.isEmpty()) {
      {
        ProcessHandler _processHandler = new Exec_Command().createProcess(file);
      ...
      {
        ProcessHandler _processHandler = new Gdb_Command().createProcess(file, project);
        final ConsoleView _consoleView = ConsoleCreator.createConsoleView(project, false);
        _processHandler.addProcessListener(new ConsoleProcessListener(_consoleView));
        return new DefaultExecutionResult(_processHandler, new DefaultExecutionConsole(_consoleView.getComponent(), new _FunctionTypes._void_P0_E0() {
          public void invoke() {
  ...
  public IDebuggerConfiguration getDebuggerConfiguration() {
    return new IDebuggerConfiguration() {
      ...
      public IDebugger getDebugger() {
        return Debuggers.getInstance().getDebuggerByName("Cpp");
      }
    };
  }
