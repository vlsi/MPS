      ...
      public ISequence<SNode> select(final SNode c) {
        return Sequence.fromIterable(parameters).where(new IWhereFilter<SNode>() {
          public boolean accept(SNode p) {
            return eq_7ie7o0_a0a0a0a0a0a0a0a0a0a2a1(ActionCallParameter_BehaviorDescriptor.getParameterDeclaration_id4PRmqZe_o$D.invoke(p), c);
          }
        });
      }
    });
        ...
        for (SNode other : Sequence.fromIterable(group).tail(Sequence.fromIterable(group).count() - 1)) {
          {
            ...
            IErrorReporter _reporter_2309309498 = typeCheckingContext.reportTypeError(other, "Duplicated parameter: " + INamedConcept_BehaviorDescriptor.getFqName_idhEwIO9y.invoke(ActionCallParameter_BehaviorDescriptor.getParameterDeclaration_id4PRmqZe_o$D.invoke(Sequence.fromIterable(group).first())), "r:71e81c80-d7fe-47f3-91de-9281cfae8376(jetbrains.mps.console.ideCommands.typesystem)", "5582028874769074233", null, errorTarget);
          }
        }
      }
    }
