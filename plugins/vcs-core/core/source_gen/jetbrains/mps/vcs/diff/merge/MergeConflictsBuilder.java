  ...
  private boolean nodeGroupChangesSymmetric(NodeGroupChange mine, NodeGroupChange repository) {
    if (mine.getBegin() == repository.getBegin() && mine.getEnd() == repository.getEnd()) {
      if (mine.getResultEnd() - mine.getResultBegin() == repository.getResultEnd() - repository.getResultBegin()) {
        List<jetbrains.mps.smodel.SNode> myChildren = IterableUtil.asList(myMyModel.getNodeById(mine.getParentNodeId()).getChildren(mine.getRole()));
        List<jetbrains.mps.smodel.SNode> repositoryChildren = IterableUtil.asList(myRepositoryModel.getNodeById(repository.getParentNodeId()).getChildren(repository.getRole()));
