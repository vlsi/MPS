...
public abstract class RectDecoratorView extends AbstractDecoratorView<GroupView, SelectionFrameView> {
  public Property<Rectangle> bounds = new ValueProperty<Rectangle>(new Rectangle(0, 0, 0, 0));


  protected SelectionFrameView createErrorView() {
    SelectionFrameView result = new SelectionFrameView();
    result.color.set(Color.RED);
    return result;
  }

  protected void registerErrorViewSynchronizers(Mapper mapper, Mapper.SynchronizersConfiguration configuration, final SelectionFrameView errorView) {
    configuration.add(Synchronizers.forProperty(bounds, errorView.bounds));
  }
