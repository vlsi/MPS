...
public class ModelCheckerExecutor {
  ...
  public ModelCheckerExecutor(List<SModule> modules, Project project) {
    // FIXME there are no uses of this class, remove? If yes, ModelCheckerController is likely to cease as well. 
  ...
  private Runnable createProcess(final boolean spawnCommands, final ProgressMonitor monitor) {
    return new Runnable() {
      ...
      public void run() {
        ...
        SearchQuery searchQuery = new SearchQuery(new ModulesHolder(modules), getMPSProject().getScope());
