...
public class SpecificModelChecker extends SpecificChecker implements ITypeContextOwner {
  ...
  public SpecificModelChecker(@NotNull Project mpsProject) {
      ...
      for (final SNode rootNode : SModelOperations.roots(model, null)) {
        TypeContextManager.getInstance().runTypeCheckingAction(this, rootNode, new ITypechecking.Action() {
          ...
          public void run(TypeCheckingContext p0) {
            Set<IErrorReporter> iErrorReporters = checker.getErrors(rootNode, myProject.getRepository());
            for (IErrorReporter errorReporter : SetSequence.fromSet(iErrorReporters)) {
              final IErrorReporter reporter = errorReporter;
              final QuickFix_Runtime quickFix = check_7763bz_a0b0b0a0a2a0a0e0d0g(check_7763bz_a0a1a1a0a0c0a0a4a3a6(errorReporter));
  ...
  private static QuickFix_Runtime check_7763bz_a0b0b0a0a2a0a0e0d0g(QuickFixProvider checkedDotOperand) {
    if (null != checkedDotOperand) {
      return checkedDotOperand.getQuickFix();
    }
    return null;
  }
  private static QuickFixProvider check_7763bz_a0a1a1a0a0c0a0a4a3a6(IErrorReporter checkedDotOperand) {
    if (null != checkedDotOperand) {
