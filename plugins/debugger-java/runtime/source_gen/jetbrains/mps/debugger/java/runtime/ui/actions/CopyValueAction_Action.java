...
public class CopyValueAction_Action extends BaseAction {
  ...
  public void doExecute(@NotNull final AnActionEvent event, final Map<String, Object> _params) {
    ...
    AbstractUiState uiState = check_cz11mp_a0d0f(DebugActionsUtil.getDebugSession(event));
    if (uiState == null || !(uiState instanceof JavaUiState)) {
      return;
    }
    JavaUiState javaUiState = (JavaUiState) uiState;

    final ThreadReference thread = check_cz11mp_a0h0f(javaUiState.getThread());
    if (thread == null) {
      return;
    }
  ...
  private static AbstractUiState check_cz11mp_a0d0f(AbstractDebugSession checkedDotOperand) {
    if (null != checkedDotOperand) {
      return checkedDotOperand.getUiState();
    }
    return null;
  }
  private static ThreadReference check_cz11mp_a0h0f(JavaThread checkedDotOperand) {
    if (null != checkedDotOperand) {
