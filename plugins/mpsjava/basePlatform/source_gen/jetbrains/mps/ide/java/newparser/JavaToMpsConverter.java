...
public class JavaToMpsConverter {
  ...
  private SModel createModel(String pkgFqName, IFile pkgDir) {
    SModel modelDescr;
    if (myCreateInplace) {
      Tuples._2<ModelRoot, SourceRoot> place = getRootContainingDir(pkgDir);
      ...
      FolderModelFactory folderModelFactory = PersistenceRegistry.getInstance().getFolderModelFactory(FilePerRootModelPersistence.FACTORY_ID);
      modelDescr = ((DefaultModelRoot) modelRoot).createModel(folderModelFactory, sourceRoot, pkgFqName);
    } else {
      ...
      modelDescr = ((DefaultModelRoot) modelRoot).createModel(pkgFqName);
    }
  ...
  private Tuples._2<ModelRoot, SourceRoot> getRootContainingDir(IFile dir) {
    ...
    for (ModelRoot modelRoot : Sequence.fromIterable(myModule.getModelRoots())) {
      ...
      for (SourceRoot sourceRoot : ListSequence.fromList(((DefaultModelRoot) modelRoot).getSourceRoots(SourceRootKinds.SOURCES))) {
        if (FileUtil.isSubPath(sourceRoot.getAbsolutePath().getPath(), dir.getPath())) {
          return MultiTuple.<ModelRoot,SourceRoot>from(modelRoot, sourceRoot);
        }
      }
    }
    return MultiTuple.<ModelRoot,SourceRoot>from((ModelRoot) null, (SourceRoot) null);
  }
}
