...
public class JavaToMpsConverter {
  ...
  private SModel createModel(String pkgFqName, IFile pkgDir) {
    ...
    if (myCreateInplace) {
      ...
      try {
        modelDescr = ((DefaultModelRoot) modelRoot).createPerRootModel(pkgFqName, sourceRoot);
      } catch (ModelCannotBeCreatedException e) {
        myMessageHandler.handle(new Message(MessageKind.ERROR, "Failed to create model " + pkgFqName));
      }
    } else {
