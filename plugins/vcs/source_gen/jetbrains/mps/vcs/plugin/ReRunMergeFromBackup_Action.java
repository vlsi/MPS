    ...
    try {
      for (File backupFile : Sequence.fromIterable(ReRunMergeFromBackup_Action.this.getBackupFiles(_params))) {
        try {
          ...
          mine = ReRunMergeFromBackup_Action.this.selectMineModel(ModelPersistence.modelToString(((BaseSModelDescriptor) MapSequence.fromMap(_params).get("model")).getSModelInternal()), mine, _params);
          if (mine == null) {
            return;
          }
