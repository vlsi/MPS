...
public class DiskMemoryConflictResolverImpl extends DiskMemoryConflictResolver {
  ...
  private static File doBackup(IFile modelFile, SModel inMemory) {
    try {
      ...
      MergeDriverBackupUtil.writeContentsToFile(
          ModelPersistence.modelToString(((BaseSModelDescriptor) inMemory).getSModelInternal()).getBytes(FileUtil.DEFAULT_CHARSET), modelFile.getName(), tmp,
          DiskMemoryConflictResolverImpl.DiskMemoryConflictVersion.MEMORY.getSuffix());
      if (modelFile.exists()) {
        com.intellij.openapi.util.io.FileUtil.copy(new File(modelFile.getPath()),
            new File(tmp.getAbsolutePath(), modelFile.getName() + "." + DiskMemoryConflictResolverImpl.DiskMemoryConflictVersion.FILE_SYSTEM.getSuffix()));
      }
  ...
  private static void openDiffDialog(IFile modelFile, SModel inMemory) {
    ...
    final ModelDifferenceDialog dialog = new ModelDifferenceDialog(onDisk.getModelDescriptor(), inMemory, project, "Filesystem version (Read-Only)",
        "Memory Version");
