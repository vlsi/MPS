package jetbrains.mps.xmlSchema.structure;

/*Generated by MPS */

import jetbrains.mps.lang.core.structure.BaseConcept;
import jetbrains.mps.lang.core.structure.INamedConcept;
import jetbrains.mps.smodel.SNode;
import java.util.Iterator;
import java.util.List;
import jetbrains.mps.lang.core.structure.Attribute;
import jetbrains.mps.smodel.SModel;
import jetbrains.mps.smodel.SModelUtil_new;
import jetbrains.mps.project.GlobalScope;

public class EntitySet extends BaseConcept implements INamedConcept {
  public static final String concept = "jetbrains.mps.xmlSchema.structure.EntitySet";
  public static final String ENTITY_SET_NAME = "entitySetName";
  public static final String URL = "url";
  public static final String NAME = "name";
  public static final String SHORT_DESCRIPTION = "shortDescription";
  public static final String ALIAS = "alias";
  public static final String VIRTUAL_PACKAGE = "virtualPackage";
  public static final String ENTITY_BLOCK = "entityBlock";
  public static final String _$ATTRIBUTE = "_$attribute";

  public EntitySet(SNode node) {
    super(node);
  }

  public String getEntitySetName() {
    return this.getProperty(EntitySet.ENTITY_SET_NAME);
  }

  public void setEntitySetName(String value) {
    this.setProperty(EntitySet.ENTITY_SET_NAME, value);
  }

  public String getUrl() {
    return this.getProperty(EntitySet.URL);
  }

  public void setUrl(String value) {
    this.setProperty(EntitySet.URL, value);
  }

  public String getName() {
    return this.getProperty(EntitySet.NAME);
  }

  public void setName(String value) {
    this.setProperty(EntitySet.NAME, value);
  }

  public String getShortDescription() {
    return this.getProperty(EntitySet.SHORT_DESCRIPTION);
  }

  public void setShortDescription(String value) {
    this.setProperty(EntitySet.SHORT_DESCRIPTION, value);
  }

  public String getAlias() {
    return this.getProperty(EntitySet.ALIAS);
  }

  public void setAlias(String value) {
    this.setProperty(EntitySet.ALIAS, value);
  }

  public String getVirtualPackage() {
    return this.getProperty(EntitySet.VIRTUAL_PACKAGE);
  }

  public void setVirtualPackage(String value) {
    this.setProperty(EntitySet.VIRTUAL_PACKAGE, value);
  }

  public int getEntityBlocksCount() {
    return this.getChildCount(EntitySet.ENTITY_BLOCK);
  }

  public Iterator<EntityBlock> entityBlocks() {
    return this.children(EntityBlock.class, EntitySet.ENTITY_BLOCK);
  }

  public List<EntityBlock> getEntityBlocks() {
    return this.getChildren(EntityBlock.class, EntitySet.ENTITY_BLOCK);
  }

  public void addEntityBlock(EntityBlock node) {
    this.addChild(EntitySet.ENTITY_BLOCK, node);
  }

  public void insertEntityBlock(EntityBlock prev, EntityBlock node) {
    this.insertChild(prev, EntitySet.ENTITY_BLOCK, node);
  }

  public int get_$attributesCount() {
    return this.getChildCount(EntitySet._$ATTRIBUTE);
  }

  public Iterator<Attribute> _$attributes() {
    return this.children(Attribute.class, EntitySet._$ATTRIBUTE);
  }

  public List<Attribute> get_$attributes() {
    return this.getChildren(Attribute.class, EntitySet._$ATTRIBUTE);
  }

  public void add_$attribute(Attribute node) {
    this.addChild(EntitySet._$ATTRIBUTE, node);
  }

  public void insert_$attribute(Attribute prev, Attribute node) {
    this.insertChild(prev, EntitySet._$ATTRIBUTE, node);
  }

  public static EntitySet newInstance(SModel sm, boolean init) {
    return (EntitySet) SModelUtil_new.instantiateConceptDeclaration("jetbrains.mps.xmlSchema.structure.EntitySet", sm, GlobalScope.getInstance(), init).getAdapter();
  }

  public static EntitySet newInstance(SModel sm) {
    return EntitySet.newInstance(sm, false);
  }
}
