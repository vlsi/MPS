...
import jetbrains.mps.buildlanguage.SmartItemsImporter.IWalker;
...
public class TaskImporterTestCase extends TestCase {
  ...
  public void testTestTask1(){
    try {
      Map<String, Class<?>> attrs = SmartItemsImporter.getInstance().getAttributesFromTask(ANT_PATH, TEST_FOLDER,"jetbrains.mps.buildlanguage.resource.TestTask", Task.class);
  ...
  public void testGetTaskClasses(){
    try {
      ClassLoader cl = SmartItemsImporter.getInstance().createClassLoader(ANT_PATH, "platform/buildlanguage/tests/testTasks.jar");
      Set<Class<? extends Task>> taskClasses = SmartItemsImporter.getInstance().getTaskClasses(new JarFile("platform/buildlanguage/tests/testTasks.jar"), cl, Task.class);
  ...
  public void testGenerateTasks1(){
    try {
      ...
      SmartItemsImporter.getInstance().generateHierarchy(ANT_PATH, "platform/buildlanguage/tests/testTasks.jar", testWalker);
  ...
  public void testGenerateTasks2NestedElements(){
    try {
      TaskImporterTestCase.TestWalker testWalker = new TestWalker();
      SmartItemsImporter.getInstance().generateHierarchy(ANT_PATH, "platform/buildlanguage/tests/testNested.jar", testWalker);
