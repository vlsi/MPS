                  ...
                  prepareTime += TextGenUtil.withTimeTracking(new Runnable() {
                    public void run() {
                      for (GResource resource : ListSequence.fromList(currentInput)) {
                        ...
                        }), resource.module(), pa.global().properties(new ITarget.Name("Make.make"), jetbrains.mps.make.facets.Make_Facet.Target_make.Parameters.class).pathToFile()));
                        ...
                        MapSequence.fromMap(streamHandlers).put(resource, new JavaStreamHandler(resource.model(), pa.global().properties(new ITarget.Name("Make.make"), jetbrains.mps.make.facets.Make_Facet.Target_make.Parameters.class).pathToFile().invoke(output), pa.global().properties(new ITarget.Name("Make.make"), jetbrains.mps.make.facets.Make_Facet.Target_make.Parameters.class).pathToFile().invoke(FileGenerationUtil.getCachesPath(output))));
                      }
                    }
                  });
