...
public class CustomContainersRegistry {
  ...
  public List<SNode> accessibleCustomContainerDeclarations(SModel fromModel) {
    ...
    if (om != null) {
      final Collection<IModule> allVisibleModules = new GlobalModuleDependenciesManager(om).getModules(Deptype.VISIBLE);
      final Collection<Language> allUsedLanguages = new GlobalModuleDependenciesManager(om).getUsedLanguages();
      Iterable<SNode> allCustomContainers = this.primAllCustomContainers();
      ListSequence.fromList(res).addSequence(Sequence.fromIterable(allCustomContainers).where(new IWhereFilter<SNode>() {
        public boolean accept(SNode cc) {
          ...
          return SetSequence.fromIterable(allVisibleModules).contains(owner) || (owner instanceof Language && SetSequence.fromIterable(allUsedLanguages).contains((Language) owner));
        }
