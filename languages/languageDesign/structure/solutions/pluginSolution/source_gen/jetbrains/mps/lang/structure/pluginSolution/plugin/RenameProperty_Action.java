...
public class RenameProperty_Action extends BaseAction {
  ...
  protected static Log log = LogFactory.getLog(RenameProperty_Action.class);
  ...
  public void doExecute(@NotNull final AnActionEvent event, final Map<String, Object> _params) {
    try {
      ...
      ModelAccess.instance().runReadAction(new Runnable() {
        public void run() {
          newName.value = RenameDialog.getNewName(((MPSProject) MapSequence.fromMap(_params).get("project")).getProject(), SPropertyOperations.getString(((SNode) MapSequence.fromMap(_params).get("target")), "name"), "property");
        }
      });
      if (newName.value == null) {
        return;
      }
      RefactoringFacade.getInstance().execute(RefactoringContext.createRefactoringContextByName("jetbrains.mps.lang.structure.refactorings.RenameProperty", Arrays.asList("newName"), Arrays.asList(newName.value), ((SNode) MapSequence.fromMap(_params).get("target")), ((MPSProject) MapSequence.fromMap(_params).get("project"))));
    } catch (Throwable t) {
      if (log.isErrorEnabled()) {
