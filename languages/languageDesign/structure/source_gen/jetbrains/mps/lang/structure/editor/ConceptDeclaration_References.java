...
public class ConceptDeclaration_References {
  public static void setCellActions(EditorCell editorCell, SNode node, EditorContext context) {
    editorCell.setAction(CellActionType.DELETE, new ConceptDeclaration_References.ConceptDeclaration_References_DELETE(node));
    editorCell.setAction(CellActionType.BACKSPACE, new ConceptDeclaration_References.ConceptDeclaration_References_BACKSPACE(node));
  }

  public static class ConceptDeclaration_References_DELETE extends AbstractCellAction {
    /*package*/ SNode myNode;

    public ConceptDeclaration_References_DELETE(SNode node) {
      this.myNode = node;
    }
  ...
  public static class ConceptDeclaration_References_BACKSPACE extends AbstractCellAction {
    /*package*/ SNode myNode;

    public ConceptDeclaration_References_BACKSPACE(SNode node) {
      this.myNode = node;
    }
    ...
    public void execute_internal(EditorContext editorContext, final SNode node) {
      ...
      final Wrappers._T<SNode> nextReference = new Wrappers._T<SNode>(null);
      final Wrappers._T<SNode> referenceToSelect = new Wrappers._T<SNode>(null);
      ListSequence.fromList(SLinkOperations.getTargets(conceptDeclaration, "linkDeclaration", true)).where(new IWhereFilter<SNode>() {
        public boolean accept(SNode it) {
          return SPropertyOperations.hasValue(it, "metaClass", "reference", "reference");
        ...
        public Boolean combine(Boolean nodeVisited, SNode nextSibling) {
          ...
          if (!(nodeVisited)) {
            referenceToSelect.value = nextSibling;
          } else if (nextReference.value == null) {
            nextReference.value = nextSibling;
          }
          return nodeVisited;
        }
      });
      ...
      SNodeOperations.deleteNode(node);
      if (referenceToSelect.value != null) {
        SelectionUtil.selectLabelCellAnSetCaret(editorContext, referenceToSelect.value, SelectionManager.LAST_CELL, -1);
      } else if (nextReference.value != null) {
        SelectionUtil.selectLabelCellAnSetCaret(editorContext, nextReference.value, SelectionManager.FIRST_CELL, 0);
      } else {
        SelectionUtil.selectLabelCellAnSetCaret(editorContext, conceptDeclaration, "emptyRefPlaceHolder", 0);
      }
    }
