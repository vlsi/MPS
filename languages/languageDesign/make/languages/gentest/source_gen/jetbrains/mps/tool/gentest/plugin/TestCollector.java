...
public class TestCollector {
  ...
  private boolean isJunit38Test(SNode maybetest) {
    boolean isdesc = BehaviorReflection.invokeVirtual(Boolean.TYPE, maybetest, "virtual_isDescendant_7165541881557222913", new Object[]{getTestCaseClass(SNodeOperations.getModel(maybetest), maybetest)});
    boolean hasann = hasAnnotation(maybetest, getIgnoreAnnotation(SNodeOperations.getModel(maybetest), maybetest), getMPSLaunchAnnotation(SNodeOperations.getModel(maybetest), maybetest));
  ...
  private boolean hasJUnit38TestMethods(final SNode maybetest) {
    return Sequence.fromIterable(Classifier_Behavior.call_methods_5292274854859311639(maybetest)).where(new IWhereFilter<SNode>() {
      ...
      public boolean accept(SNode meth) {
        return !(hasAnnotation(meth, getIgnoreAnnotation(SNodeOperations.getModel(maybetest), maybetest)));
      }
    });
  }
  private boolean isJUnit4Test(SNode maybetest) {
    return !(hasAnnotation(getIgnoreAnnotation(SNodeOperations.getModel(maybetest), maybetest), getMPSLaunchAnnotation(SNodeOperations.getModel(maybetest), maybetest))) && hasJUnit4TestMethods(maybetest);
  }
  private boolean hasJUnit4TestMethods(final SNode maybetest) {
    return Sequence.fromIterable(Classifier_Behavior.call_methods_5292274854859311639(maybetest)).where(new IWhereFilter<SNode>() {
      public boolean accept(SNode meth) {
        return "public".equals(SPropertyOperations.getString(SNodeOperations.getConceptDeclaration(SLinkOperations.getTarget(meth, MetaAdapterFactory.getContainmentLink(0xf3061a5392264cc5L, 0xa443f952ceaf5816L, 0x112670d273fL, 0x112670d886aL, "visibility"))), MetaAdapterFactory.getProperty(0xc72da2b97cce4447L, 0x8389f407dc1158b7L, 0x1103553c5ffL, 0x46ab0ad5826c74caL, "conceptAlias"))) && hasAnnotation(meth, getTestAnnotation(SNodeOperations.getModel(maybetest), maybetest));
      }
    }).any(new IWhereFilter<SNode>() {
      public boolean accept(SNode meth) {
        return !(hasAnnotation(meth, getIgnoreAnnotation(SNodeOperations.getModel(maybetest), maybetest)));
  ...
  private SNode getTestCaseClass(final SModel context, final SNode maybetest) {
    return SNodeOperations.cast(getNode(context, TEST_CASE, new _FunctionTypes._return_P0_E0<SNode>() {
      public SNode invoke() {
        ...
        return SNodeOperations.cast(ListSequence.fromList(TestCollector.this.allClassifiers(context, maybetest)).findFirst(new IWhereFilter<SNode>() {
          public boolean accept(SNode cls) {
  ...
  private SNode getTestAnnotation(final SModel context, final SNode maybetest) {
    return SNodeOperations.cast(getNode(context, TEST_ANN, new _FunctionTypes._return_P0_E0<SNode>() {
      public SNode invoke() {
        ...
        return ListSequence.fromList(TestCollector.this.allAnnotations(context, maybetest)).findFirst(new IWhereFilter<SNode>() {
  ...
  private SNode getIgnoreAnnotation(final SModel context, final SNode maybetest) {
  ...
  private SNode getMPSLaunchAnnotation(final SModel context, final SNode maybetest) {
  ...
  protected List<SNode> allClassifiers(SModel context, SNode maybetest) {
    Iterable<SNode> result = Sequence.fromIterable(new ClassifiersScope(context, maybetest, MetaAdapterFactory.getConcept(0xf3061a5392264cc5L, 0xa443f952ceaf5816L, 0x101d9d3ca30L, "jetbrains.mps.baseLanguage.structure.Classifier"), true).getAvailableElements("")).select(new ISelector<SNode, SNode>() {
  ...
  protected List<SNode> allAnnotations(SModel context, SNode maybetest) {
    Iterable<SNode> result = Sequence.fromIterable(new ClassifiersScope(context, maybetest, MetaAdapterFactory.getConcept(0xf3061a5392264cc5L, 0xa443f952ceaf5816L, 0x114a69dc80cL, "jetbrains.mps.baseLanguage.structure.Annotation"), true).getAvailableElements("")).select(new ISelector<SNode, SNode>() {
      public SNode select(SNode it) {
