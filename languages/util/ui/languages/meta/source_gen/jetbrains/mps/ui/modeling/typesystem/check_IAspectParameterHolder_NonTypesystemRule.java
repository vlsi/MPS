...
public class check_IAspectParameterHolder_NonTypesystemRule extends AbstractNonTypesystemRule_Runtime implements NonTypesystemRule_Runtime {
  ...
  public void applyRule(final SNode hld, final TypeCheckingContext typeCheckingContext, IsApplicableStatus status) {
    if (!(ListSequence.<SNode>fromList(SLinkOperations.getTargets(SLinkOperations.getTarget(SNodeOperations.getAncestor(hld, "jetbrains.mps.ui.modeling.structure.ContainerTemplate", false, false), "container", false), "requiredAspect", true)).disjunction(ListSequence.<SNode>fromList(SLinkOperations.getTargets(hld, "aspectParameter", true)).<SNode>select(new ISelector<SNode, SNode>() {
