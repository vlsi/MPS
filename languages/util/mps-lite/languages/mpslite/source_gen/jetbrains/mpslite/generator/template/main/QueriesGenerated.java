...
public class QueriesGenerated {
  public static void mappingScript_CodeBlock_1238589718396(final IOperationContext operationContext, final MappingScriptContext _context) {
    ...
    Map<SNode, SNode> conceptsToTargets = MapSequence.<SNode,SNode>fromMap(new HashMap<SNode, SNode>());
    Map<SNode, SNode> additionalConceptsToTargets = MapSequence.<SNode,SNode>fromMap(new HashMap<SNode, SNode>());
    Map<SNode, SNode> partsToLinkDeclarations = MapSequence.<SNode,SNode>fromMap(new HashMap<SNode, SNode>());
    SNode conceptContainer = ListSequence.<SNode>fromList(SModelOperations.getRoots(_context.getModel(), "jetbrains.mpslite.structure.ConceptContainer")).first();
    ...
    for (SNode conceptDeclaration : allConcepts) {
      ...
      if (IMPSLiteConcept_Behavior.call_isAbstract_1239715026284(conceptDeclaration)) {
        ...
        SNode conceptProperty_Abstract = SNodeOperations.cast(ListSequence.<SNode>fromList(SLinkOperations.getTargets(SConceptOperations.findConceptDeclaration("jetbrains.mps.lang.core.structure.BaseConcept"), "conceptPropertyDeclaration", true)).where(new IWhereFilter<SNode>() {
      ...
      MapSequence.<SNode,SNode>fromMap(conceptsToTargets).put(conceptDeclaration, concept);
    }
    // additional concepts 
    for (SNode conceptDeclaration : allConcepts) {
      ...
      if (concept != null) {
        MapSequence.<SNode,SNode>fromMap(additionalConceptsToTargets).put(conceptDeclaration, concept);
      }
    }
    // extends 
    for (SNode conceptDeclaration : allConcepts) {
      SLinkOperations.setTarget(((SNode) MapSequence.<SNode,SNode>fromMap(conceptsToTargets).get(conceptDeclaration)), "extends", ((SNode) AbstractConceptReference_Behavior.call_getConcept_1238594571574(SLinkOperations.getTarget(conceptDeclaration, "extends", true), conceptsToTargets)), false);
    }
    // inner concept structure 
    for (SNode conceptDeclaration : allConcepts) {
      IMPSLiteConcept_Behavior.call_fillConcept_1239891562930(conceptDeclaration, ((SNode) MapSequence.<SNode,SNode>fromMap(conceptsToTargets).get(conceptDeclaration)), conceptsToTargets, partsToLinkDeclarations);
    }
    // editor 
    ...
    Map<SNode, SNode> conceptsToEditors = MapSequence.<SNode,SNode>fromMap(new HashMap<SNode, SNode>());
    for (SNode conceptDeclaration : allConcepts) {
      ...
      SNode mpsConcept = MapSequence.<SNode,SNode>fromMap(conceptsToTargets).get(conceptDeclaration);
      SLinkOperations.setTarget(editor, "conceptDeclaration", mpsConcept, false);
      MapSequence.<SNode,SNode>fromMap(conceptsToEditors).put(mpsConcept, editor);
    }
    for (SNode additionalConcept : MapSequence.fromMap(additionalConceptsToTargets).keySet()) {
      ...
      SNode mpsConcept = MapSequence.<SNode,SNode>fromMap(additionalConceptsToTargets).get(additionalConcept);
      SLinkOperations.setTarget(editor, "conceptDeclaration", mpsConcept, false);
      MapSequence.<SNode,SNode>fromMap(conceptsToEditors).put(mpsConcept, editor);
    }
    ...
    // constraints(scopes) 
    Map<SNode, SNode> conceptsToConstraints = MapSequence.<SNode,SNode>fromMap(new HashMap<SNode, SNode>());
    for (SNode variableConcept : ConceptContainer_Behavior.call_getVariableConcepts_1239806150736(conceptContainer)) {
      ...
      SLinkOperations.setTarget(conceptConstraint, "concept", MapSequence.<SNode,SNode>fromMap(additionalConceptsToTargets).get(variableConcept), false);
      ...
      ListSequence.<SNode>fromList(SLinkOperations.getTargets(conceptConstraint, "referent", true)).addElement(varScope);
      MapSequence.<SNode,SNode>fromMap(conceptsToConstraints).put(variableConcept, conceptConstraint);
    }
    ...
    for (SNode root : ListSequence.<SNode>fromListWithValues(new ArrayList<SNode>(), SModelOperations.getRoots(structureModel, null))) {
      structureModel.removeRoot(root);
    }
    for (SNode conceptDeclaration : allConcepts) {
      SNode concept = MapSequence.<SNode,SNode>fromMap(conceptsToTargets).get(conceptDeclaration);
    ...
    for (SNode conceptDeclaration : MapSequence.fromMap(additionalConceptsToTargets).keySet()) {
      SNode concept = MapSequence.<SNode,SNode>fromMap(additionalConceptsToTargets).get(conceptDeclaration);
      SModelOperations.addRootNode(structureModel, concept);
    }
    structureModel.setLoading(false);
    editorModel.setLoading(true);
    for (SNode root : ListSequence.<SNode>fromListWithValues(new ArrayList<SNode>(), SModelOperations.getRoots(editorModel, null))) {
      editorModel.removeRoot(root);
    }
    for (SNode conceptDeclaration : allConcepts) {
      SNode editorDeclaration = MapSequence.<SNode,SNode>fromMap(conceptsToEditors).get(MapSequence.<SNode,SNode>fromMap(conceptsToTargets).get(conceptDeclaration));
      SModelOperations.addRootNode(editorModel, editorDeclaration);
    }
    for (SNode conceptDeclaration : MapSequence.fromMap(additionalConceptsToTargets).keySet()) {
      SNode editorDeclaration = MapSequence.<SNode,SNode>fromMap(conceptsToEditors).get(MapSequence.<SNode,SNode>fromMap(additionalConceptsToTargets).get(conceptDeclaration));
      SModelOperations.addRootNode(editorModel, editorDeclaration);
    }
    editorModel.setLoading(false);
    actionsModel.setLoading(true);
    for (SNode root : ListSequence.<SNode>fromListWithValues(new ArrayList<SNode>(), SModelOperations.getRoots(actionsModel, null))) {
    ...
    for (SNode root : ListSequence.<SNode>fromListWithValues(new ArrayList<SNode>(), SModelOperations.getRoots(constraintsModel, null))) {
      constraintsModel.removeRoot(root);
    }
    for (SNode conceptDeclaration : allConcepts) {
      SNode conceptConstraints = MapSequence.<SNode,SNode>fromMap(conceptsToConstraints).get(conceptDeclaration);
