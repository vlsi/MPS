...
import jetbrains.mps.buildScript.behavior.BuildMacro_Behavior;
...
public final class MacroHelper {
  ...
  private void add(SNode macro, String importName, String exportName) {
    SNode macroProject = SNodeOperations.as(SNodeOperations.getContainingRoot(macro), "jetbrains.mps.buildScript.structure.BuildProject");
  ...
  public Iterable<SNode> getVarsContainers() {
    return ListSequence.fromList(SLinkOperations.getTargets(this.project, "dependencies", true)).where(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return SNodeOperations.isInstanceOf(it, "jetbrains.mps.buildScript.structure.BuildProjectDependency") && depPrefixes.containsKey(SNodeOperations.cast(it, "jetbrains.mps.buildScript.structure.BuildProjectDependency"));
      }
    }).select(new ISelector<SNode, SNode>() {
      public SNode select(SNode it) {
        return SNodeOperations.cast(it, "jetbrains.mps.buildScript.structure.BuildProjectDependency");
  ...
  public static class MacroContext {
    ...
    public MacroHelper getMacros(SNode dep) {
      dep = SNodeOperations.as(DependenciesHelper.getOriginalNode(dep, genContext), "jetbrains.mps.buildScript.structure.BuildProject");
      if (dep == null) {
        return null;
      }
    ...
    public SNode getOriginalMacro(SNode macro) {
      return SNodeOperations.as(DependenciesHelper.getOriginalNode(macro, genContext), "jetbrains.mps.buildScript.structure.BuildMacro");
    }

    public SNode getOriginalDep(SNode dep) {
      return SNodeOperations.as(DependenciesHelper.getOriginalNode(dep, genContext), "jetbrains.mps.buildScript.structure.BuildProjectDependency");
    }
