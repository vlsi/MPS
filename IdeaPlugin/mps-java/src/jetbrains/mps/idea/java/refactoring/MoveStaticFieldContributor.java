...
public class MoveStaticFieldContributor implements MoveRefactoringContributor {
  static String REFACTORING_CLASS = "jetbrains.mps.baseLanguage.refactorings.MoveStaticField";
    ...
    MoveStaticFieldExecute.execute(mpsProject, target, new ExecuteRefactoring() {
      ...
      public void run(SNode whereToMove) {
        RefactoringAccess.getInstance().getRefactoringFacade().execute(
          RefactoringContext.createRefactoringContext(new MovetStaticFieldRefactoring(),
  ...
  class MovetStaticFieldRefactoring extends PsiAwareRefactoring {
    public MovetStaticFieldRefactoring() {
    ...
    public void refactor(RefactoringContext refactoringContext) {
      ...
      try {
        ...
        SNode field = refactoringContext.getSelectedNode();

        final PsiField psiField = (PsiField) psiProvider.getPsi(field);
        ...
        for (SearchResult<SNode> result : usages.getSearchResults()) {
          ...
          MoveMembersUsageInfo moveUsageInfo = handler.getUsage(psiField, psiRef, new HashSet<PsiMember>(Arrays.asList(psiField)), targetClass);
