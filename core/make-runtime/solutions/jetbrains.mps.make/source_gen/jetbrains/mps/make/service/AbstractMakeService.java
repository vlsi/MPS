...
public abstract class AbstractMakeService implements IMakeService {
  ...
  protected abstract class AbstractInputProcessor {
    ...
    protected final Future<IResult> processRawInput(final Iterable<? extends IResource> inputRes, final IScript defaultScript, IScriptController controller) {
      ...
      Iterable<ScriptBuilder> scriptBuilders = Sequence.<Iterable<String>>fromIterable(usedLangs.value).<ScriptBuilder>select(new ISelector<Iterable<String>, ScriptBuilder>() {
        public ScriptBuilder select(Iterable<String> langs) {
          final ScriptBuilder scb = new ScriptBuilder();
          Sequence.<String>fromIterable(langs).visitAll(new IVisitor<String>() {
            public void visit(String ns) {
              ...
              scb.withFacetNames(Sequence.<IFacet>fromIterable(fcts).<IFacet.Name>select(new ISelector<IFacet, IFacet.Name>() {
      ...
      Iterable<IScript> scripts = ((defaultScript != null ?
        Sequence.<ScriptBuilder>fromIterable(scriptBuilders).<IScript>select(new ISelector<ScriptBuilder, IScript>() {
