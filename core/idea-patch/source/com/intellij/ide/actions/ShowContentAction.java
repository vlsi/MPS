...
public class ShowContentAction extends AnAction implements DumbAware {
  ...
  public void update(AnActionEvent e) {
    e.getPresentation().setEnabled(isEnabled(getWindow(e)));
  }

  @Patch
  private boolean isEnabled(ToolWindow window) {
    if (window == null) return false;
    if (!(window instanceof ToolWindowImpl)) return true;
    return !((ToolWindowImpl) window).getId().equals(InspectorTool.ID);
  }
