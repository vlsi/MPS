...
// Patched by MPS in order to fix botva with run and debug tool windows
// The problem is that RunContentManager wont give our actions run content descriptor
// method getSelectedContent() just returns null

// because myToolwindowZbuffer ALWAYS has Run tool first and Debug tool the second.
// May be not the best solution of the problem but the only that seems to be available for now.
