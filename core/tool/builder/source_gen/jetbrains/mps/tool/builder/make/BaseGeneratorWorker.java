...
import jetbrains.mps.make.script.IScriptController.Stub;
import jetbrains.mps.make.service.AbstractMakeService;
import jetbrains.mps.make.service.AbstractMakeService.DefaultMonitor;
...
public class BaseGeneratorWorker extends MpsWorker {
  ...
  protected void generate(Project project, MpsWorker.ObjectsToProcess go) {
    ...
    DefaultMonitor defaultMonitor = new DefaultMonitor(session);
    final JavaCompilerProperties properties = new JavaCompilerProperties(myWhatToDo);
    Stub controller = new Stub(defaultMonitor, defaultMonitor) {
      @Override
      public void setup(IPropertiesPool ppool) {
        new JavaCompileFacetInitializer().setJavaCompileOptions(new JavaCompilerOptions(JavaVersion.parse(properties.getTargetJavaVersion()))).populate(ppool);
      }
    };
