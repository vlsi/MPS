...
public class JavaStackFrame extends ProxyForJava implements IStackFrame {
    ...
    try {
      ...
      if (stackFrame != null) {
        ...
        for (LocalVariable variable : map.keySet()) {
          result.put(new JavaLocalVariable(variable, this, myClassFqName, stackFrame.thread()), AbstractValueUtil.getInstance().fromJDIValue(map.get(variable), myClassFqName, stackFrame.thread()));
        }
  ...
  public IValue getValue(IWatchable watchable) {
    if (watchable instanceof JavaLocalVariable) {
      ...
      return AbstractValueUtil.getInstance().fromJDIValue(getStackFrame().getValue(localVariable.getLocalVariable()), myClassFqName, myThreadReference);
    }
    return null;
  }
