package jetbrains.mps.lang.plugin.typesystem;

/*Generated by MPS */

import jetbrains.mps.lang.typesystem.runtime.quickfix.QuickFix_Runtime;
import jetbrains.mps.lang.plugin.behavior.ICheckedNamePolicy_Behavior;
import jetbrains.mps.smodel.SNode;
import jetbrains.mps.lang.smodel.generator.smodelAdapter.SPropertyOperations;
import jetbrains.mps.util.NameUtil;
import jetbrains.mps.checkedName.PropertyReference;

public class FixNamingPolicy_QuickFix extends QuickFix_Runtime {

  public FixNamingPolicy_QuickFix() {
  }

  public String getDescription() {
    return ICheckedNamePolicy_Behavior.call_getFixIntentionCaption_1628770029971140579(((SNode)this.getField("nodeToFix")[0]));
  }

  public void execute(SNode node) {
    for(SNode s : ICheckedNamePolicy_Behavior.call_getDescendantsToCheck_1628770029971140562(((SNode)this.getField("nodeToFix")[0]))) {
      SPropertyOperations.set(s, "value", NameUtil.captionPartWithNamingPolicy(SPropertyOperations.getString(s, "value")));
    }
    for(PropertyReference p : ICheckedNamePolicy_Behavior.call_getPropertiesToCheck_1628770029971140570(((SNode)this.getField("nodeToFix")[0]))) {
      String value = p.getNode().getProperty(p.getProperty());
      String newValue = NameUtil.captionWithNamingPolicy(value);
      p.getNode().setProperty(p.getProperty(), newValue);
    }
  }

}
