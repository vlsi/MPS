    ...
    RulesUtil.equate_inputNodeConcept(node, TypeChecker.getInstance().getEquationManager().getRepresentator(Concept_typevar_1205967749950));
    {
      ...
      TypeChecker.getInstance().getRuntimeSupport().whenConcrete(C, new Runnable() {
        ...
        public void run() {
          ...
          TypeChecker.getInstance().getRuntimeSupport().createEquation(TypeChecker.getInstance().getRuntimeSupport().typeOf(node, "jetbrains.mps.bootstrap.smodelLanguage.helgins", "1205967749966", true), new QuotationClass_79().createNode(inputNodeConcept), node, null, "jetbrains.mps.bootstrap.smodelLanguage.helgins", "1205967749964");
        }

      }, "jetbrains.mps.bootstrap.smodelLanguage.helgins", "1205967749957");
    }
    TypeChecker.getInstance().getRuntimeSupport().createLessThanInequation(TypeChecker.getInstance().getRuntimeSupport().typeOf(node, "jetbrains.mps.bootstrap.smodelLanguage.helgins", "1186057865109", true), new QuotationClass_48().createNode(), node, null, "jetbrains.mps.bootstrap.smodelLanguage.helgins", "1186057869956", false);
  }
