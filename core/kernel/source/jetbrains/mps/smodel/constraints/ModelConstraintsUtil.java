...
public class ModelConstraintsUtil {
  ...
  public static SearchScopeStatus getSearchScope(SNode enclosingNode, SNode referenceNode, SNode referenceNodeConcept, SNode referenceLinkDeclaration, SNode enclosingLinkDeclaration, IOperationContext context) {
    ...
    return getSearchScope(enclosingNode, referenceNode, referenceNodeConcept, linkRole, linkTarget, enclosingLinkDeclaration, context);
  }

  /**
   * @param linkRole - use *genuine* link role here!!!
   */
  public static SearchScopeStatus getSearchScope(SNode enclosingNode, SNode referenceNode, SNode referenceNodeConcept, String linkRole, SNode enclosingLinkDeclaration, IOperationContext context) {
    return getSearchScope(enclosingNode, referenceNode, referenceNodeConcept, linkRole, null, enclosingLinkDeclaration, context);
  }

  private static SearchScopeStatus getSearchScope(SNode enclosingNode, final SNode referenceNode, final SNode referenceNodeConcept, final String linkRole, final SNode linkTarget, final SNode enclosingLinkDeclaration, final IOperationContext context) {
    final SModel model;
    ...
    ModelAccess.instance().runReadAction(new Runnable() {
      public void run() {
        ...
        TypeContextManager.getInstance().runResolveAction(new Runnable() {
          ...
          public void run() {
            try {
              status[0] = getSearchScope_intern(model, enclosingNode_, referenceNode, referenceNodeConcept, linkRole, linkTarget, enclosingLinkDeclaration, context);
            } catch (Exception t) {
  ...
  private static SearchScopeStatus getSearchScope_intern(
    ...
    String linkRole,
    SNode linkTarget,
    SNode enclosingLinkDeclaration,
    IOperationContext context) {

    INodeReferentSearchScopeProvider scopeProvider = getSearchScopeProvider(referenceNodeConcept, linkRole);
    ReferentConstraintContext referentConstraintContext = new ReferentConstraintContext(model, enclosingNode, referenceNode, linkTarget, enclosingLinkDeclaration);
  ...
  public static SearchScopeStatus createSearchScope(final INodeReferentSearchScopeProvider scopeProvider,
    ...
    final ReferentConstraintContext referentConstraintContext = new ReferentConstraintContext(model, enclosingNode, referenceNode, linkTarget, enclosingLinkDeclaration);
    try {
  ...
  public static IReferencePresentation getPresentation(SNode enclosingNode, SNode referenceNode, SNode referenceNodeConcept, SNode referenceLinkDeclaration, SNode enclosingLinkDeclaration, IOperationContext context) {
    String linkRole = SModelUtil.getGenuineLinkRole(referenceLinkDeclaration);
    SNode linkTarget = SModelUtil.getLinkDeclarationTarget(referenceLinkDeclaration);
    final SModel model;
    if (enclosingNode != null) {
      ...
      if (myProvider != null) {
        return myProvider.getPresentation(myOperationContext,
          new PresentationReferentConstraintContext(myContext.getModel(), myContext.getEnclosingNode(),
            myContext.getReferenceNode(), myContext.getLinkTarget(), node, myContext.getEnclosingLink(), visible, smartRef, inEditor));
      }
