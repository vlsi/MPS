...
public abstract class BaseEditableSModelDescriptor extends BaseSModelDescriptorWithSource implements EditableSModelDescriptor {
  ...
  public void resolveDiskConflict() {
    LOG.warning("Model=" + getSModel().getSModelReference().getSModelFqName() + ", file ts=" + getSource().getTimestamp() + ", model ts=" + getSourceTimestamp(), new Throwable());  // more information
  ...
  public boolean checkAndResolveConflictOnSave() {
    if (needsReloading()) {
      LOG.warning("Model file " + getSModel().getSModelReference().getSModelFqName() + " was modified externally!\n" +
