...
class ModelChange {
  static void assertLegalNodeChange(SModel model, SNode node) {
    ...
    if (((ABCDE) model).canFireEvent() && node.getModel() != null && !UndoHelper.getInstance().isInsideUndoableCommand()) {
  ...
  static void assertLegalNodeRegistration(SModel model, SNode node) {
    if (((ABCDE) model).canFireEvent() && !UndoHelper.getInstance().isInsideUndoableCommand()) {
  ...
  static void assertLegalChange(SModel model) {
    if (((ABCDE) model).canFireEvent() && !ModelAccess.instance().canWrite()) {
  ...
  static boolean needFireEvents(SModel model, SNode node) {
    return node.getModel() != null && ((ABCDE) model).canFireEvent();
  }
}
