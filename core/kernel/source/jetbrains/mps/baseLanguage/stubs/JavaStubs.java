...
public class JavaStubs extends AbstractModelRootManager {
  ...
  public Collection<SModelDescriptor> load(@NotNull ModelRoot root, IModule module) {
    ...
    getModelDescriptors(result, cp, root.getPrefix(), LanguageID.JAVA, module);
    return result;
  }

  private static void getModelDescriptors(Set<SModelDescriptor> result, IClassPathItem cp, String prefix, String languageId, IModule module) {
    for (String subpackage : cp.getSubpackages(prefix)) {
      if (cp.getRootClasses(subpackage).iterator().hasNext()) {
        ...
        if (SModelRepository.getInstance().getModelDescriptor(modelReference) != null) {
          ...
          result.add(new BaseStubModelDescriptor(modelReference, new JavaStubModelDataSource(), module));
        }
      }
      getModelDescriptors(result, cp, prefix + '.' + subpackage, languageId, module);
    }
  }
