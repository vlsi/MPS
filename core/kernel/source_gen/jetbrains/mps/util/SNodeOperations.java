...
public class SNodeOperations {
  ...
  public static boolean isUnknown(SNode node) {
    SConcept conceptId = node.getConceptId();
    if (conceptId == null) {
      return true;
    }
    // sNode.getConceptId() might be a non-null value for illegal concept, make sure the concept is known 
    SAbstractConcept concept = SConceptRepository.getInstance().getConcept(conceptId);
    if (concept == null) {
      return true;
    }
    ...
    SModule sourceModule = MetaIdByDeclaration.getModuleReference(conceptId.getLanguage()).resolve(MPSModuleRepository.getInstance());
