...
public class SortedSet_Test extends Util_Test {
  public void test_order() throws Exception {
    SortedSet<Integer> set = SortedSetSequence.<Integer>fromSetAndArray(new TreeSet<Integer>(), 4, 5, 3, 2, 1);
    this.assertIterableEquals(this.input5(), set);
    Assert.assertSame(1, SortedSetSequence.<Integer>fromSet(set).first());
    Assert.assertSame(5, SortedSetSequence.<Integer>fromSet(set).last());
  }

  public void test_multiOrder() throws Exception {
    SortedSet<Integer> set = SortedSetSequence.<Integer>fromSetAndArray(new TreeSet<Integer>(), 2, 1, 2, 5, 4, 3, 1, 5, 3, 2, 3, 1);
    this.assertIterableEquals(this.input5(), set);
    Assert.assertSame(1, SortedSetSequence.<Integer>fromSet(set).first());
    Assert.assertSame(5, SortedSetSequence.<Integer>fromSet(set).last());
  }
  ...
  public void test_headSet() throws Exception {
    SortedSet<String> set = SortedSetSequence.<String>fromSetWithValues(new TreeSet<String>(), this.inputABCDEF());
  ...
  public void test_comparator() throws Exception {
    SortedSet<String> sset = SortedSetSequence.<String>fromSet(new TreeSet<String>(new Comparator<String>() {
      public int compare(String a, String b) {
        return -String.CASE_INSENSITIVE_ORDER.compare(a, b);
      }
    }));
    SortedSetSequence.fromSet(sset).addSequence(Sequence.<String>fromIterable(Sequence.fromArray(new String[]{"b", "d", "c", "a"})));
