...
public class TypeUtil_Collections {
  ...
  public static SNode coerceTo_SequenceType(SNode type) {
    SNode sequenceType = TypeChecker.getInstance().getRuntimeSupport().coerce(type, HUtil.createMatchingPatternByConceptFQName("jetbrains.mps.baseLanguage.collections.structure.SequenceType"), true);
    ...
    if ("jetbrains.teamsys.dnq.structure.PListType".equals(type.getConceptFqName())) {
      ...
      return new _Quotations.QuotationClass_41().createNode(SNodeOperations.copyNode(classifierType));
    }
    // END TEMP FIX
    // ==========
    // ==========
    ...
    if (type != null) {
      ...
      if (TypeChecker.getInstance().getSubtypingManager().isSubtype(type, javaIterableType)) {
        return new _Quotations.QuotationClass_42().createNode(SNodeOperations.copyNode(entity));
      }
      // ==========
      ...
      if (TypeChecker.getInstance().getSubtypingManager().isSubtype(type, entityIterableType)) {
        return new _Quotations.QuotationClass_43().createNode(SNodeOperations.copyNode(entity));
      }
    }
    // END FIX
    // ==========
