...
public class QueueSequence<T> extends CollectionSequence<T> implements Queue<T>, IQueueSequence<T>, Serializable {
    ...
    public static <U> IQueueSequence<U> fromQueueAndArray(Queue<U> queue, U... array) {
        if (Sequence.USE_NULL_SEQUENCE) {
            if (queue == null && array == null) {
                ...
                if (queue instanceof IQueueSequence) { return (IQueueSequence<U>) queue; }
                return new QueueSequence<U>(queue);
            }
        }
    ...
    public static <U> IQueueSequence<U> fromQueueWithValues(Queue<U> queue, Iterable<U> it) {
        ...
        if (Sequence.USE_NULL_SEQUENCE) {
            ...
            else if (it == null) { return fromQueue(queue); }
        }
        if (Sequence.IGNORE_NULL_VALUES) {
            for (U u : it) {
                if (u != null) {
        ...
        if (tmp instanceof IQueueSequence) { return (IQueueSequence<U>) tmp; }
