...
__switch__:
  ...
  private static void inference_mapTypeVariables(final TypeCheckingContext typeCheckingContext, SNode type, Map<SNode, List<SNode>> mmap) {
    ...
    {
      List<SNode> children = ListSequence.<SNode>fromArray();
      ListSequence.fromList(children).addSequence(ListSequence.fromList(SNodeOperations.getChildren(type)));
    ...
    for(SNode statementList : statementLists) {
      ...
      if (SNodeOperations.isInstanceOf(parent, "jetbrains.mps.baseLanguage.structure.BaseMethodDeclaration") && SLinkOperations.getTarget(parent, "body", true) == statementList) {
        ...
        if (!(throwables.isEmpty())) {
          ...
          {
            BaseIntentionProvider intentionProvider = null;
            intentionProvider = new BaseIntentionProvider("r:ed6478fc-2afe-47f0-bab4-a562aa3c7d79(jetbrains.mps.baseLanguage.typesystem@17_0).AddExceptionToMethodSignature_QuickFix", false);
