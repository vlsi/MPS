...
public abstract class GenericDeclarationTextGen2 extends BaseLanguageTextGen {
  public static void typeDeclarations(SNode generic, final SNodeTextGen textGen) {
    if (ListSequence.<SNode>fromList(SLinkOperations.getTargets(generic, "typeVariableDeclaration", true)).isNotEmpty()) {
      textGen.append("<");
      if (ListSequence.<SNode>fromList(SLinkOperations.getTargets(generic, "typeVariableDeclaration", true)).isNotEmpty()) {
        for (SNode item : SLinkOperations.getTargets(generic, "typeVariableDeclaration", true)) {
          ...
          if (item != ListSequence.<SNode>fromList(SLinkOperations.getTargets(generic, "typeVariableDeclaration", true)).last()) {
            textGen.append(", ");
          }
        }
      }
