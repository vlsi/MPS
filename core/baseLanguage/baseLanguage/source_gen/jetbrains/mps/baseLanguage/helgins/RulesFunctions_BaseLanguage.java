...
public class RulesFunctions_BaseLanguage {
  ...
  public static void numericOp(final SNode binOp) {
    ...
    TypeChecker.getInstance().getRuntimeSupport().createLessThanInequation(TypeChecker.getInstance().getEquationManager().getRepresentator(rightExpressionType_typevar_1185962758265), SLinkOperations.getTarget(new QuotationClass_26().createNode(), "descriptor", false), SLinkOperations.getTarget(binOp, "rightExpression", true), null, "jetbrains.mps.baseLanguage.helgins", "1176908856117", false);
    TypeChecker.getInstance().getRuntimeSupport().createLessThanInequation(TypeChecker.getInstance().getEquationManager().getRepresentator(leftExpressionType_typevar_1185962769627), SLinkOperations.getTarget(new QuotationClass_27().createNode(), "descriptor", false), SLinkOperations.getTarget(binOp, "leftExpression", true), null, "jetbrains.mps.baseLanguage.helgins", "1176908902743", false);
    {
        ...
        for(SNode child : SNodeOperations.getDescendants(returnType, null, true)) {
          if (SNodeOperations.isInstanceOf(child, "jetbrains.mps.baseLanguage.structure.TypeVariableReference")) {
            ...
            SNode actualParam = (index < actualParams.size() && index >= 0 ?
              ...
              new QuotationClass_28().createNode()
            );
            if (returnType == tvr) {
    ...
    for(SNode statementList : statementLists) {
      ...
      if (SNodeOperations.isInstanceOf(parent, "jetbrains.mps.baseLanguage.structure.BaseMethodDeclaration") && SLinkOperations.getTarget(parent, "body", true) == statementList) {
        ...
        if (!(throwables.isEmpty())) {
          ...
          TypeChecker.getInstance().reportTypeError(mainNode, errorString, "jetbrains.mps.baseLanguage.helgins", "1210182111558", "jetbrains.mps.baseLanguage.helgins.AddExceptionToMethodSignature_Intention");
        }
        return;
      }
    }
