...
public class CompiledJava extends AbstractModelRootManager {
  public CompiledJava() {
  }
  ...
  public Collection<SModelDescriptor> load(@NotNull ModelRoot root, IModule module) {
    ...
    IClassPathItem cp = CompiledJava.this.create(root.getPath());
    CompiledJava.this.getModelDescriptors(result, root.getPath(), cp, (root.getPrefix() == null ?
      "" :
      root.getPrefix()
  ...
  /*package*/ void getModelDescriptors(List<SModelDescriptor> result, String startPath, IClassPathItem cp, String prefix, String languageId, IModule module) {
    for (String subpackage : cp.getSubpackages(prefix)) {
      if (cp.getRootClasses(subpackage).iterator().hasNext()) {
        ...
        smd.getSource().addPath(CompiledJava.this.child(startPath, subpackage));
      }
      CompiledJava.this.getModelDescriptors(result, startPath, cp, subpackage, languageId, module);
    }
  }
