...
import jetbrains.mps.baseLanguage.plugin.JavaApplication_ConfigurationType;
...
public class LegacyRunConfigurationsProvider_CustomApplicationPlugin extends BaseCustomApplicationPlugin {
  ...
  public void doInit() {
    ...
    ConfigurationType configurationType_ = ContainerUtil.findInstance(Extensions.getExtensions(ConfigurationType.CONFIGURATION_TYPE_EP), new _FunctionTypes._return_P0_E0<Class<ConfigurationType>>() {
      public Class<ConfigurationType> invoke() {
        try {
          ...
          LOG.error("Can not find configuration type " + "jetbrains.mps.baseLanguage.plugin.JavaApplication_ConfigurationType" + ". Check languages dependency.", cl);
          return (Class<ConfigurationType>) null;
        }
      }
    }.invoke());
    if (configurationType_ != null) {
      ((JavaApplication_ConfigurationType) configurationType_).addForeignFactory(new JavaNodeApplication_Factory(configurationType_));
    ...
    {
      JavaNodeApplicationFromNode configurationCreator = new JavaNodeApplicationFromNode();
