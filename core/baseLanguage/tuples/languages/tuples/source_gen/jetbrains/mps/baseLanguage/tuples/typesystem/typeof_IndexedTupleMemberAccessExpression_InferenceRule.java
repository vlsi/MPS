    ...
    if (Expression_Behavior.call_isCompileTimeConstant_1238860258777(SLinkOperations.getTarget(mae, "index", true))) {
      ...
      );
      {
        ...
        typeCheckingContext.whenConcrete(tupleType, new Runnable() {
          public void run() {
            if (!(index >= 0 && index < ListSequence.fromList(SLinkOperations.getTargets(SNodeOperations.as(typeCheckingContext.getExpandedType(tupleType), "jetbrains.mps.baseLanguage.tuples.structure.IndexedTupleType"), "componentType", true)).count())) {
              MessageTarget errorTarget = new NodeMessageTarget();
            ...
            if (index >= 0 && index < ListSequence.fromList(SLinkOperations.getTargets(SNodeOperations.as(typeCheckingContext.getExpandedType(tupleType), "jetbrains.mps.baseLanguage.tuples.structure.IndexedTupleType"), "componentType", true)).count()) {
              List<SNode> mtypes = SLinkOperations.getTargets(SNodeOperations.as(typeCheckingContext.getExpandedType(tupleType), "jetbrains.mps.baseLanguage.tuples.structure.IndexedTupleType"), "componentType", true);
              {
