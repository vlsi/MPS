...
public class ConstraintsChecker extends SpecificChecker {
  public ConstraintsChecker(ModelChecker modelChecker) {
    super(modelChecker);
  }

  public void checkModel(SModel model) {
    for (SNode node : ListSequence.fromList(SModelOperations.getNodes(model, null))) {
      if (!(this.getModelChecker().checkAndUpdateIndicator("Checking " + SModelOperations.getModelName(model) + " for cardinalities and properties constraints..."))) {
        return;
      }
        ...
        if (LinkDeclaration_Behavior.call_isAtLeastOneCardinality_3386205146660812199(link)) {
          if (SPropertyOperations.hasValue(link, "metaClass", "aggregation", "reference")) {
            if (ListSequence.fromList(SNodeOperations.getChildren(node, link)).isEmpty()) {
              this.addIssue(node, "Cardinality constraint violation in role \"" + SPropertyOperations.getString(link, "role") + "\"");
            }
          } else {
      ...
      for (SNode child : ListSequence.fromList(SNodeOperations.getChildren(node)).where(new IWhereFilter<SNode>() {
        ...
        if (!(ModelCheckerUtils.isDeclaredLink(SNodeOperations.getContainingLinkDeclaration(child), true))) {
          this.addIssue(node, "Usage of undeclared child role \"" + SNodeOperations.getContainingLinkRole(child) + "\"", ModelChecker.CATEGORY_WARNING, new ModelCheckerFix.UndeclaredChild(node, SNodeOperations.getContainingLinkRole(child)));
        }
      }
      ...
      for (SReference reference : ListSequence.fromList(SNodeOperations.getReferences(node))) {
        if (!(ModelCheckerUtils.isDeclaredLink(SLinkOperations.findLinkDeclaration(reference), false))) {
          this.addIssue(node, "Usage of undeclared reference role \"" + reference + "\"", ModelChecker.CATEGORY_WARNING, new ModelCheckerFix.UndeclaredReference(node, SLinkOperations.getRole(reference)));
        }
      }
      ...
      for (PropertyDeclaration p : ListSequence.fromList(props)) {
        ...
        if (!(ps.canSetValue(node, p.getName(), value, this.getModelChecker().getOperationContext().getScope()))) {
          this.addIssue(node, "Property constraint violation for property \"" + p.getName() + "\"", ModelChecker.CATEGORY_WARNING, null);
        }
      }
      ...
      for (String name : SetSequence.fromSet(node.getPropertyNames())) {
        ...
        if (!(isDeclaredProperty(concept, name))) {
          this.addIssue(node, "Usage of undeclared property \"" + name + "\"", ModelChecker.CATEGORY_WARNING, new ModelCheckerFix.UndeclaredProperty(node, name));
        }
      }
    }
  }
