...
public class ModelProperties extends BaseBean {
  ...
  public ModelProperties(SModelDescriptor modelDescriptor, IOperationContext context) {
    ...
    myContext = context;
    myImportedModels.addAll(SModelOperations.getImportedModelUIDs(myModelDescriptor.getSModel()));
    myUsedLanguages.addAll(myModelDescriptor.getSModel().importedLanguages());
    myUsedDevKits.addAll(myModelDescriptor.getSModel().importedDevkits());
    myLanguagesEngagedOnGeneration.addAll(myModelDescriptor.getSModel().engagedOnGenerationLanguages());
  ...
  private void addNewLanguages() {
    ...
    for (ModuleReference namespace : languagesInProps) {
      ...
      if (language != null) {
        ...
        SModelOperations.addLanguage(myModelDescriptor.getSModel(), language);
      }
    }
  }
  ...
  private void addNewModels() {
    Set<SModelReference> modelsInModel = new HashSet<SModelReference>(SModelOperations.getImportedModelUIDs(myModelDescriptor.getSModel()));
    ...
    for (SModelReference modelReference : modelsInProps) {
      SModelOperations.addImportedModel(myModelDescriptor.getSModel(), modelReference);
    }
  }
  ...
  private void removeUnusedModels() {
    ...
    for (SModelReference modelReference : modelsInModel) {
      myModelDescriptor.getSModel().deleteModelImport(modelReference);
    }
  }
}
