...
public class WorkbenchMakeService implements IMakeService {
  private Logger LOG = Logger.getLogger(WorkbenchMakeService.class);
  ...
  private IResult doMake(final Iterable<? extends IResource> inputRes, IScript script, IMakeService.Executor executor) {
    if (Sequence.fromIterable(inputRes).isEmpty()) {
      if (cleanMake) {
        ...
        displayInfo("Everything up to date");
  ...
  private void displayInfo(String msg) {
    IdeFrame ideFrame = WindowManager.getInstance().getIdeFrame(context.getProject());
    if (ideFrame != null) {
      ideFrame.getStatusBar().setInfo(msg);
    }
    else {
      LOG.info(msg);
    }
