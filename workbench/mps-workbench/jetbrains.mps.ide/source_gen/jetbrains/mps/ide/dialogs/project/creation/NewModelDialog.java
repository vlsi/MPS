...
public class NewModelDialog extends DialogWrapper {
  ...
  private void initContentPane() {
    ...
    myModelStorageFormat.setRenderer(new ColoredListCellRenderer() {
      protected void customizeCellRenderer(JList p0, Object p1, int p2, boolean p3, boolean p4) {
        if (!(p1 instanceof ModelFactoryType)) {
          return;
        }
        append(((ModelFactoryType) p1).getFormatTitle());
      }
    });
  ...
  private ModelFactoryType[] getStorageFormats() {
    List<ModelFactoryType> list = ModelFactoryService.getInstance().getFactoryTypes();
    return list.toArray(new ModelFactoryType[list.size()]);
  }
  @Override
